...
#[get("/")]
fn show_login(cookies: &Cookies) -> Result<io::Result<NamedFile>, Redirect> {
    if let Some(_) = get_id_from_session(&cookies) {
        Err(Redirect::to("/"))
    } else {
...
fn login(cookies: &Cookies, user_login: Form<UserLogin>) -> Redirect {
    ...
    let user: User = user_login.get();
    let api_key = session::generate_api_key();
    save_to_redis(api_key.clone(), user.id);
    let cookie: Cookie = Cookie::new("doc-session".to_string(), api_key);
    cookies.add(cookie);
    Redirect::to("/")
}
...
#[get("/")]
fn show_register(cookies: &Cookies) -> Result<io::Result<NamedFile>, Redirect> {
    if let Some(_) = get_id_from_session(&cookies) {
        Err(Redirect::to("/"))
    } else {
...
fn new_document(cookies: &Cookies, document_form: Form<DocumentForm>, folder_name: &str) -> Redirect {
    if let Some(user_id) = get_id_from_session(&cookies) {
        let document_new = document_form.into_inner();
        if !document_new.validate() {
            return Redirect::to("/")
        }
...
fn main() {
    rocket::ignite().mount("/",         routes![dashboard_redirect, static_files::all])
                    .mount("/home",     routes![redirect_home, new_document, show_folder, show_document])
